<template>
  <view class="container">
            ...
            <text class="text-list">
              {{
                wxs.parseDiscountMsg(
                  item.chooseDiscountItemDto.discountRule,
                  item.chooseDiscountItemDto.needAmount,
                  item.chooseDiscountItemDto.discount
                )
              }}
            </text>
          </view>
            ...
            <view class="item">
              ...
              <view class="prodinfo">
                ...
                <view class="opt">
                  ...
                  <text
                    :class="'prod-info-text ' + (prod.skuName ? '' : 'empty-n')"
                  >
                    {{ prod.skuName }}
                  </text>
    ...
    <view v-if="!shopCartItemDiscounts.length" class="empty">
      ...
      <view class="txt"> 您还没有添加任何农品哦~ </view>
    </view>
...
const onSelectedItem = (e) => {
  ...
  const checked =
    shopCartItemDiscountsParam[scindex].shopCartItems[index].checked; // 获取当前商品的选中状态
...
const onCountMinus = (e) => {
  ...
  const shopCartItemDiscountsParam = shopCartItemDiscounts.value;
  const prodCount =
    shopCartItemDiscountsParam[scindex].shopCartItems[index].prodCount;
  if (prodCount > 1) {
