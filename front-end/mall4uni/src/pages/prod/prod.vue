<template>
  ...
  <view class="container">
    ...
    <!-- 评价 -->
    <view class="prod-comm">
      <view class="title">商品评价</view>
      <view v-if="!commList.length" class="empty">暂无评价</view>
      <view v-else class="comm-list">
        <view v-for="comm in commList" :key="comm.commId" class="comm-item">
          <view class="user-info">
            <img-show
              :src="comm.isAnonymous ? '' : comm.pic"
              :default-img-type="true"
              class-list="['avatar']"
            />
            <text class="username">{{
              comm.isAnonymous ? "匿名用户" : comm.nickName
            }}</text>
          </view>
          <view class="comm-content">
            <uni-rate :value="comm.score" readonly />
            <text class="content">{{ comm.content }}</text>
            <view class="pics">
              <img-show
                v-for="pic in comm.pics"
                :key="pic"
                :src="pic"
                class-list="['pic']"
              />
            </view>
            <text class="time">{{ comm.recTime }}</text>
          </view>
        </view>
      </view>
    </view>
...
const getProdComm = () => {
  http
    ...
    .then(({ data }) => {
      commList.value = data.records.map((comm) => {
        try {
          comm.pics = comm.pics ? JSON.parse(comm.pics) : [];
        } catch (e) {
          comm.pics = [];
        }
        return comm;
 ...
 * 判断当前的规格值 是否可以选
 */
const isSkuLineItemNotOptional = (
  allProperties,
  selectedPropObjParam,
  key,
  item,
  propKeys
) => {
